...
DEFAULT_DISPLAY=":99"
DEFAULT_ROBOT_TEST="-i healthdist"
INSTALL_NAME="OpenECOMP_ETE"
DEFAULT_OUTPUT_FOLDER=/share/logs/healthdist/
...
VARIABLES="-v ziplist:/share/heat/temp/vFW.zip -v  catalog_service_name:Health$today"
...
python -m robot.run -L ${LOG_LEVEL} -d ${OUTPUT_FOLDER} ${VARIABLEFILES} ${VARIABLES} ${LISTENERS} -P ${ROBOT_LIBS} ${ROBOT_TAGS} -t "Health Distribution Test" $(pwd)/robot/testsuites/model-distribution-health.robot
RET_CODE=$?
