...
${CL_DATA_PATH}                     /policy/api/v1/policytypes/onap.policies.controlloop.operational.common.Drools/versions/1.0.0/policies
${MONITORING_DATA_PATH}             /policy/api/v1/policytypes/onap.policies.monitoring.docker.sonhandler.app/versions/1.0.0/policies
...
${RESOURCES_PATH}                   ${EXECDIR}/robot/assets/usecases/5gson
${CONFIGDB_BLUEPRINT_PATH}          ${EXECDIR}/robot/assets/usecases/5gson/k8s-configdb.yaml
...
Create dmaap topics
    [Tags]                              5gson
    :FOR   ${topic}   IN   @{TOPICS}
    \   ${data}=                        Set Variable                       {"owner": "", "txenabled": false,"topicName": "${topic}"}
    \   ${data_path}=                   Set Variable                       /topics/create
    \   ${resp}=                        Run MR Auth Post Request           data_path=${data_path}    id_key=""    secret_key=""     data=${data}
...
Deploy SON Handler
    ...
    ${sonhms_inputs}=                   Get Binary File                    ${RESOURCES_PATH}/sonhms_inputs.json
...
Deploy Config DB
    [Tags]                              5gson
    ${configdb_blueprint_path}          Set Variable                       ${RESOURCES_PATH}/k8s-configdb.yaml
...
Load Data to Config DB
    [Tags]                              5gson
    Sleep                               30 seconds
    Run Process                         wget                               https://raw.githubusercontent.com/onap-oof-pci-poc/sdnc/master/ConfigDB/Dublin/RAN_DumpFile_15Mar19.json     --output-document\=/tmp/dump_file.json
    ${initial_dump}                     Get Binary File                    /tmp/dump_file.json
    ${headers}=                         Create Dictionary                  content-type=application/json
