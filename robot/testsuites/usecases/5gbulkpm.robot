...
Setting Global Variables
    ...
    Set Global Variable   ${GLOBAL_TEST_VARAIBLES}  ${test_variables}
...
Uploading PM Files to xNF SFTP Server After Services Deployed
    [Tags]                              5gbulkpm                           5gbulkpm_checking_sftp_rsa_key
    ${pm_file}=  Upload PM Files to xNF SFTP Server  ${GLOBAL_TEST_VARAIBLES["PM_FILE_PATH"]}  ${BULK_PM_MODE}
...
Sending File Ready Event to VES Collector After Services Deployed
    [Tags]                              5gbulkpm                 5gbulkpm_checking_sftp_rsa_key
    Send File Ready Event to VES Collector  ${PM_FILE}  ${GLOBAL_TEST_VARAIBLES["FILE_FORMAT_TYPE"]}  ${GLOBAL_TEST_VARAIBLES["FILE_FORMAT_VERSION"]}

Verifying 3GPP Perf VES Content On PERFORMANCE_MEASUREMENTS Topic After Services Deployed
    [Tags]                              5gbulkpm                            5gbulkpm_checking_sftp_rsa_key
    Wait Until Keyword Succeeds         2 min                            5 sec            xNF PM File Validate   ${BULK_PM_MODE}   ${GLOBAL_TEST_VARAIBLES["EXPECTED_PM_STR"]}  ${GLOBAL_TEST_VARAIBLES["EXPECTED_EVENT_JSON_PATH"]}
