...
Deploying Data File Collector
    [Tags]                              5gbulkpm                           5gbulkpm_checking_key
...
Setting Known_Hosts Environment Set
    [Tags]                             5gbulkpm_checking_key
    Run and Return RC                  ${SET_KNOWN_HOSTS_FILE_PATH}
    Wait Until Keyword Succeeds        5 min                               10s               Check Known Hosts In Env             ${CHECK_ENV_SET}
    Run and Return RC                  ${GET_RSA_KEY}
    Run and Return RC                  ${COPY_RSA_KEY}
...
Checking DFC Logs After Known_Hosts Set
    ...
    Should Not Contain               ${dfc_logs}                             StrictHostKeyChecking is enabled but environment variable KNOWN_HOSTS_FILE_PATH is not set or points to not existing file
