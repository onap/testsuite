...
Upload PM Files to xNF SFTP Server After Services Deployed
    ...
    ${check_key_existing}=   Run Keyword And Return Status  Get From Dictionary  ${ENV_VARIABLES}  BULK_PM_MODE

    ${ftp_file_path}=   Set Variable If  "${check_key_existing}" == "True"  ${ENV_VARIABLES["PM_FILE_PATH"]}  ${FTP_FILE_PATH}

    ${epoch}=  Upload PM Files to xNF SFTP Server  ${ftp_file_path}
    Set Global Variable  ${EPOCH}  ${epoch}
...
Sending File Ready Event to VES Collector After Services Deployed
    ...
    ${check_key_existing}=   Run Keyword And Return Status  Get From Dictionary  ${ENV_VARIABLES}  BULK_PM_MODE
    ${file_format_type}=   Set Variable If  "${check_key_existing}" == "True"  ${ENV_VARIABLES["FILE_FORMAT_TYPE"]}  ${DEFAULT_ENV_VARIABLES["FILE_FORMAT_TYPE"]}

    Send File Ready Event to VES Collector  ${EPOCH}  ${file_format_type}
