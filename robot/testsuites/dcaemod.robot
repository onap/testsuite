...
Test Setup       Usecase Setup
Test Teardown    Usecase Teardown  ${is_process_group_set}  ${is_flow_distributed_set}  ${typeId}  ${blueprintName}  ${is_bp_deployed_set}  ${processGroupId}
...
Deploy DCAE Simple Application Without Config Map In Config Spec Json
    ...
    ${dict_values} =  Create Dictionary  comp_spec_name=withoutcmp
    ...
    ${processGroupId} =  Create Process Group  kasia123
    Set Test Variable  ${is_process_group_set}  True
    Create Processor  ${processGroupId}  ${compSpecName}
    Save Flow By Version Controlling  kasia123  ${processGroupId}
    Distribute The Flow  ${processGroupId}
    Set Test Variable  ${is_flow_distributed_set}  True
    ${typeId}  ${blueprintName} =  Deploy Blueprint From Inventory  kasia123  ${compSpecName}
