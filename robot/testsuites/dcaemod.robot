...
Deploy DCAE Simple Application Without Config Map In Config Spec Json
    ...
    ${dict_values} =  Create Dictionary  comp_spec_name=tescik
    ${compSpecName} =  Set Variable  ${dict_values['comp_spec_name']}
    Set Test Variable  ${processGroupName}  tescik
...
Deploy DCAE Simple Application With Config Map In Config Spec Json AND Present In K8s
   # [tags]              dcaemod
    ...
    [Teardown]  Run Keywords  Test Teardown - Delete Process Group And Blueprint And Deployment  AND  Delete Config Map With Mounted Config Fie
