...
Deploy DCAE Simple Application Without Config Map In Config Spec Json
    ...
    ${dict_values} =  Create Dictionary  comp_spec_name=without
    ...
    ${processGroupId} =  Create Process Group  sylwia1234
    ...
    Save Flow By Version Controlling  sylwia1234  ${processGroupId}
    Distribute The Flow  ${processGroupId}
    Set Test Variable  ${is_flow_distributed_set}  True
    ${typeId}  ${blueprintName} =  Deploy Blueprint From Inventory  sylwia1234  ${compSpecName}
    #Set Test Variable  ${SERVICE_TYPE_ID}  ${typeId}
    #Set Test Variable  ${BLUEPRINT_NAME}  ${blueprintName}
    Set Test Variable  ${is_bp_deployed_set}  True

    [Teardown]  Specific Test Teardown  ${is_process_group_set}  ${is_flow_distributed_set}  ${typeId}  ${blueprintName}  ${is_bp_deployed_set}  ${processGroupId}
