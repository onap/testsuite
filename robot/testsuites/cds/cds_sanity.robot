...
${CONTROLLER_BLUEPRINT_ENDPOINT}    ${GLOBAL_CCSDK_CDS_SERVER_PROTOCOL}://${GLOBAL_INJECTED_CCSDK_CDS_CONTROLLER_BLUEPRINT_IP_ADDR}:${GLOBAL_CCSDK_CDS_HEALTH_SERVER_PORT}
...
#WIP to test enrich with any service provided
Use list instead of repeating test case
    [Documentation]    Runs a check on CDS enrich API
    ${files}=    OperatingSystem.List Directory    ${PATH_CBA}    *.zip    absolute=${TRUE}
    : For    ${file}    in    @{files}
    \    ${file_}=    Evaluate    {'file': open('${file}', 'r')}
    \    ${resp}=    CDS Post Request with files   url=${CONTROLLER_BLUEPRINT_ENDPOINT}${VERSIONED_INDEX_PATH}blueprint-model/enrich    files=${file_}
    \    Should Be Equal As Strings  ${resp.status_code}    200
