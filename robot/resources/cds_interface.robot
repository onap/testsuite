...
${CDS_RETRIEVE_DATA_DICTIONARY_ENDPOINT}             /api/v1/dictionary/search/restmock
...
${CREATE_DICTIONARY_JSON_PATH}                       /var/opt/ONAP/robot/assets/cds/create_dictionary.json
${BOOTSTRAP_JSON_PATH}                               /var/opt/ONAP/robot/assets/cds/bootstrap.json
${CDS_CBA_PROCESS_FILE_PATH}                         /var/opt/ONAP/robot/assets/cds/cba_process.json
${CDS_CBA_PACKAGE_FILE}                              /var/opt/ONAP/robot/assets/cds/cba.zip
${CDS_CBA_ENRICHED_FILE}                             /var/opt/ONAP/robot/assets/cds/enriched-cba.zip
${SUCCESS}                                           EVENT_COMPONENT_EXECUTED
${EMPTY_VALUE_TO_RESOLVE}                            ${EMPTY}
...
Run CDS GET Data Dictionary Health Check
    ...
    ${session}=    Create Session       cds    ${CDS_HEALTH_ENDPOINT}    auth=${auth}
    ${headers}=  Create Dictionary    Accept=application/json    Content-Type=application/json
    ${resp}=    Get Request     cds    ${CDS_RETRIEVE_DATA_DICTIONARY_ENDPOINT}     headers=${headers}
    Log    Received response code from cds ${resp}
    Log    Received content from cds ${resp.content}
    Should Be Equal As Strings  ${resp.status_code}     200
    ${res_body}=   Convert to string     ${resp.content}
    ${file}   Get Binary File    ${CREATE_DICTIONARY_JSON_PATH}
    ${name_validate}    Evaluate    json.loads('''${file}''')    json
    Log    ${name_validate["name"]}
    Should Contain   ${res_body}     ${name_validate["name"]}
...
Run CDS Publish CBA Health Check
    ...
    Should Be Equal As Strings  ${resp.status_code}    200      And    ${resp.json()['blueprintModel']['id']}!= ${EMPTY}
