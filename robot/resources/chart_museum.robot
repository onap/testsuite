...
Install helm charts
    ...
    [Arguments]                             ${chart_repo_name}                      ${dcae_servcie_helm_charts}         ${dcae_service_helm_name}       ${wait_time}=2 min
    ${helm_install}=                        Set Variable                            helm install ${dcae_service_helm_name} ${chart_repo_name}/${dcae_servcie_helm_charts} --set global.repository=${registry_ovveride}
...
Install helm charts from folder
    ...
    [Arguments]                             ${chart_folder}                         ${dcae_service_helm_name}       ${wait_time}=2 min
    ${helm_install}=                        Set Variable                            helm install ${dcae_service_helm_name} ${chart_folder} --set global.repository=${registry_ovveride}
    ${helm_install_command_output} =        Run And Return Rc And Output            ${helm_install}
    Should Be Equal As Integers             ${helm_install_command_output[0]}       0
    Wait Until Keyword Succeeds             ${wait_time}                            20 sec                             Checking Status Of Deployed Appliction Using Helm      ${dcae_servcie_helm_charts}                 ${dcae_service_helm_name}
...
Checking Status Of Deployed Appliction Using Helm
    ...
    ${pod_status}=                      Set Variable                                kubectl get pods -n onap | grep ${dcae_service_helm_name}-${dcae_servcie_helm_charts} | awk '{print $3}'
    ...
    ${pod_ready}=                       Set Variable                                kubectl get pods -n onap | grep ${dcae_service_helm_name}-${dcae_servcie_helm_charts} | awk '{print $2}'
