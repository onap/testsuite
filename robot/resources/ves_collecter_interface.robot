...
Send VES Event
    ...
    ${vnfName}=  Catenate   vFWCntLoopHealth1
    Wait Until Keyword Succeeds   60   15    Check VES Event on DMaaP   ${vnfName}
    Wait Until Keyword Succeeds   60   15    Check TCA Event on DMaaP   ${vnfName}
Check VES Event on DMaaP
    [Arguments]   ${vnfName}
    ...
    Log To Console   -
    Log To Console   Successful VES Event on DMaaP
Check TCA Event on DMaaP
    [Arguments]   ${vnfName}
    ${resp}=    Run MR Get Request   events/unauthenticated.DCAE_CL_OUTPUT/groupR19/CR1?timeout=5000
    #${resp}=    Run MR Get Request   events/unauthenticated.TCA_EVENT_OUTPUT/groupR19/CR1?timeout=5000
    Should Be Equal As Strings 	${resp.status_code} 	200
    Should Contain   ${resp.text}   ${vnfName}
    Log To Console   Successful TCA Event on DMaaP
