...
Load CNF Customer and Models
    ...
    [Arguments]    ${customer_name}
    ${customer_id}=     Generate UUID4
    ${tenant_id}=       Generate UUID4
    Load VID Entities   ${customer_name}
...
Load CNF Customer and Service
    ...
    [Arguments]     ${customer_name}    ${customer_id}  ${customer_type}    ${cloud_region_id}  ${tenant_id}    ${tenant_name}  ${service_name}

    Inventory Tenant If Not Exists      ${GLOBAL_AAI_CNF_CLOUD_OWNER}  ${cloud_region_id}  k8s
    ...
    ...                                 ${GLOBAL_AAI_CNF_CLOUD_OWNER}   ${cloud_region_id}  ${GLOBAL_AAI_CLOUD_OWNER_DEFINED_TYPE}
    Create Service If Not Exists        ${service_name}

    ${arguments}=   Create Dictionary       subscriber_name=${customer_name}    global_customer_id=${customer_id}   subscriber_type=${customer_type}
    ...                                     cloud_owner1=${GLOBAL_AAI_CNF_CLOUD_OWNER}  cloud_region_id1=${cloud_region_id}     tenant_id1=${tenant_id}
    ...
    ${catdb_template_arguments}=    Create Dictionary   site_name=${GLOBAL_AAI_CNF_CLOUD_OWNER}  region_id=${cloud_region_id}
