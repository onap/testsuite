...
${BC_ENDPOINT}     http://${GLOBAL_INJECTED_BC_IP_ADDR}:${GLOBAL_BC_SERVER_PORT}
...
Run BC Health Check
     ...
     ${resp}=    Return dmaap details    ${BC_HEALTH_CHECK_PATH}
...
Return dmaap details
     [Documentation]    Runs Bus Controler get details request
     [Arguments]    ${data_path}
     ${session}=    Create Session      bs      ${BC_ENDPOINT}
