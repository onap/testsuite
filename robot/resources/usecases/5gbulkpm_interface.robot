...
Validate Json Event From Topic
    [Arguments]  ${expected_event_json_path}  ${expected_pm_str}
    ${PmEventRespFiles} =  OperatingSystem.List Files In Directory  ${expected_event_json_path}  *.json
    Set Test Variable  ${pmResultFound}  False
    FOR  ${fileName}  IN  @{PmEventRespFiles}
         ${filePath} =  Join Path  ${expected_event_json_path}  ${fileName}
         Set Log Level  NONE
...
Get Event From Topic
    [Arguments]  ${expected_event_json_path}
    Wait Until Keyword Succeeds  2 min  5 s  xNF PM File Validate  none  ${expected_event_json_path}
...
xNF PM File Validate
    ...
    Run Keyword If  '${expected_event_json_path}' != 'none'  Create File  ${expected_event_json_path}/pm_event-${timestamp}.json  ${pmEventJson}
