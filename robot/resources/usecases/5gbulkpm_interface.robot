...
Validate Json Event From Topic
    [Arguments]  ${expected_result_path}  ${topic_output}
    ${pm_even_resp_files} =  OperatingSystem.List Files In Directory  ${expected_result_path}  *.json
    ${pmResultFound} =  Set Variable  False
    FOR  ${file_name}  IN  @{pm_even_resp_files}
         ${filePath} =  Join Path  ${expected_result_path}  ${file_name}
         ${pm_data} =  OperatingSystem.Get File  ${filePath}
         ${statusOfPmResult}=  Run Keyword And Return Status  Should Contain  ${pm_data}  ${topic_output}
         Run Keyword If  '${statusOfPmResult}' == 'True'  Run Keywords  Set Variable  ${pmResultFound}  True
         ...                                                       AND  Exit For Loop
    END

   Should Be True  ${pmResultFound}


Get Event From Topic
    [Arguments]  ${expected_result_path}
    Wait Until Keyword Succeeds  2 min  5 s  xNF PM File Validate  none  ${expected_result_path}


xNF PM File Validate
    [Arguments]  ${topic_output}  ${expected_result_path}
    #Run Keyword If  '${expected_result_path}' != 'none'  Set Log Level  NONE
    ...
    ${json}=  Run Keyword If  '${expected_result_path}' != 'none'  To Json  ${resp.text}  pretty_print=True
    Run Keyword If  '${expected_result_path}' != 'none'  Create File  ${expected_result_path}/pm_event-${timestamp}.json     ${json}
    #Run Keyword If  '${expected_result_path}' != 'none'  Set Log Level  INFO
    Run Keyword If  '${expected_result_path}' == 'none'  Should Contain  ${resp.text}  ${topic_output}
