...
Validate Json Event From Topic
    ...
    ${json}  =  Load JSON From File  ${expected_result_path}
    ${json_string} =  Stringify Json  ${json}
    Should Contain  ${json_string}  ${topic_output}

Get Event From Topic
    [Arguments]  ${expected_result_path}
    Wait Until Keyword Succeeds  2 min  5 s  xNF PM File Validate  ${expected_result_path}


xNF PM File Validate
    [Arguments]  ${topic_output}=none  ${expected_result_path}=none
    ...
    Run Keyword If  '${expected_result_path}' != 'none'  Append To File  ${expected_result_path}     ${json}
