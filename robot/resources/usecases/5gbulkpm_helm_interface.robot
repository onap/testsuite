...
Delete PERFORMANCE_MEASUREMENTS topic
    ${resp}=                            Run MR Delete Request                  webapi/topics/org.onap.dmaap.mr.PERFORMANCE_MEASUREMENTS
...
Deploying Data File Collector
    Install helm charts                 chart-museum                       dcae-datafile-collector         ${ONAP_HELM_RELEASE}-dcae-datafile-collector           3 min      --set useCmpv2Certificates=true --set global.cmpv2Enabled=true --set masterPasswordOverride=test
...
Change DFC httpsHostnameVerify configuration in Consul
    ...
    Should Be Equal As Integers   ${rc}                           0
    Wait Until Keyword Succeeds         120 sec          5 sec       Check DFC logs                  kubectl logs -n onap $(kubectl get pods -n onap | grep datafile-collector | awk '{print $1}' | grep -v NAME) ${container_name}-datafile-collector
