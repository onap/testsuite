...
Deploying HTTPS server with correct certificates
    ${name} =                           Set Variable                       ${ONAP_HELM_RELEASE}-pm-https-server-correct-sans --debug
...
Deploying HTTPS server with wrong certificates - wrong SAN-s
    ${name} =                           Set Variable                       ${ONAP_HELM_RELEASE}-pm-https-server-wrong-sans --debug
...
Check logs
    [Arguments]  ${LOG_CHECK}
    ${rc} =                                    Run And Return Rc And Output                    ${DFC_LOG_CHECK}
