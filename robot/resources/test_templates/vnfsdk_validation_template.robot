...
Create CSAR Package
    ...
    ${folder}=    Catenate    ${VNFSDK_TOSCA_ONBOARDING_PACKAGES_DIRECTORY}/${package_folder}
#    Create Zip From Files In Directory        ${folder}    ${csar}    sub_directories=${true}
    ${rc} =     Run and Return RC       cd ${folder}; zip -r ${csar} *
    Should Be Equal As Integers         ${rc}    0
    [Return]  ${csar}

Create CSAR Package For CMS Only
    [Arguments]  ${package_folder}
    ${csar}=    Catenate    ${VNFSDK_CSAR_DIRECTORY}/${package_folder}.csar
    ${folder}=    Catenate    ${VNFSDK_CSAR_DIRECTORY}/${package_folder}
#    Create Zip From Files In Directory        ${folder}    ${csar}    sub_directories=${true}
    ${rc} =     Run and Return RC       cd ${folder}; zip -r ${csar} *
    Should Be Equal As Integers         ${rc}    0
    [Return]  ${csar}
...
Create Secured CSAR ZIP Package with CMS
    ...
    Copy Directory  ${VNFSDK_TOSCA_ONBOARDING_PACKAGES_DIRECTORY}/${package_folder}  ${VNFSDK_CSAR_DIRECTORY}
    @{files}=  List Files In Directory  ${VNFSDK_CSAR_DIRECTORY}/${package_folder}  *.mf
    ${mf_file_name}=  Set Variable  @{files}[0]
    ${mf}=   Catenate   ${VNFSDK_CSAR_DIRECTORY}/${package_folder}/${mf_file_name}
    ${rc} =     Run and Return RC   openssl cms -sign -signer /tmp/package-robot-${cert}.cert -inkey /tmp/package-private-robot-${cert}.key -outform PEM -binary -in ${mf} -out ${cms}
    Should Be Equal As Integers         ${rc}    0
    ${rc} =     Run and Return RC   cat ${cms} >> ${mf}
    Log File  ${mf}
    Should Be Equal As Integers         ${rc}    0
    Remove File  ${cms}
    ${csar}=  Create CSAR Package For CMS Only  ${package_folder}
