...
Instantiate PNF_macro service and succesfully registrate PNF template
    ...
    ${gen_uuid}=   Generate UUID4
    Load OwningEntity  project  Project-${customer_name}-${gen_uuid}
    Load OwningEntity  owningEntity  OE-${customer_name}-${gen_uuid}
    Load OwningEntity  lineOfBusiness  LOB-${customer_name}-${gen_uuid}
    Load OwningEntity  platform  Platform-${customer_name}-${gen_uuid}
    ${service_instance_id}  ${request_id}  ${full_customer_name}   Run Keyword If  "${building_block_flow}"=='false'  Orchestrate PNF Macro Flow   ${customer_name}   ${service}    ${product_family}  ${pnf_correlation_id}  ${tenant_id}   ${tenant_name}  ${service_name}  ${region}  Project-${customer_name}-${gen_uuid}   OE-${customer_name}-${gen_uuid}
        ...  ELSE  Orchestrate PNF Building Block Flow   ${catalog_service_name}  ${customer_name}    ${service}    ${product_family}    ${pnf_correlation_id}   ${region}   project_name=Project-${customer_name}-${gen_uuid}   owning_entity=OE-${customer_name}-${gen_uuid}  lineOfBusinessName=LOB-${customer_name}-${gen_uuid}   platformName=Platform-${customer_name}-${gen_uuid}
