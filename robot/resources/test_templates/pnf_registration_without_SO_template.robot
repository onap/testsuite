...
Design, create, instantiate PNF/macro service and succesfully registrate PNF template
    ...
    ${status}   ${value}=   Run Keyword And Ignore Error   Distribute TOSCA Based PNF Model  ${service}  ${service_name}
    ...
    ...  ELSE  Log To Console  Check TOSCA Based PNF Model Distribution
    ...
    Send VES integration request  ${PNF_entry_dict}
    Verify PNF Integration Request in A&AI  ${PNF_entry_dict}
    Wait Until Keyword Succeeds   10s  5s  Check SO service completition status   ${request_id}   COMPLETE
