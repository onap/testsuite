...
Onboard Component Spec
    [Arguments]  ${componentSpec}  ${dict_values}
    ${session}=  Create Session   ${SESSION_NAME}  ${DCAEMOD_SERVER}
    ${headers}=  Create Dictionary  content-type=application/json
...
Create Processor
    ...
    ${session}=  Create Session   ${SESSION_NAME}  ${DCAEMOD_SERVER}
    ${headers}=  Create Dictionary  content-type=application/json
    ${data} =  Set Variable  {"revision": {"clientId": "${CLIENT_ID}", "version": 0},"component": {"parentGroupId": "${processGroupId}", "name": "${COMPONENT_NAME}", "type": "org.onap.dcae.${COMPONENT_NAME}"}}
...
Deploy Blueprint From Inventory
    ...
    ${compSpecName} =  Convert To Lower Case  ${compSpecName}
    ${blueprintName} =  Set Variable  ${compSpecName}_${COMPONENT_NAME}
...
Delete Process Group
    ...
    ${session}=  Create Session   ${SESSION_NAME}  ${DCAEMOD_SERVER}
    ${currentProcessGrVersion}  ${clientId}=  Get Process Group Revision  ${PROCESS_GROUP_ID}
...
Usecase Teardown
    Run Keyword If  ${IS_DISTRIBUTION_TARGET_SET}  Wait Until Keyword Succeeds  2 min  5s  Delete Distribution Target
    Run Keyword If  ${IS_REGISTRY_CLIENT_SET}      Wait Until Keyword Succeeds  2 min  5s  Delete Registry Client
    Run Keyword If  ${IS_PROCESS_GROUP_SET}        Wait Until Keyword Succeeds  2 min  5s  Delete Process Group
    Run Keyword If  ${IS_FLOW_DISTRIBUTED_SET}     Delete Blueprint From Inventory  ${SERVICE_TYPE_ID}
    Run Keyword If  ${IS_BP_DEPLOYED_SET}          Undeploy Service  ${BLUEPRINT_NAME}