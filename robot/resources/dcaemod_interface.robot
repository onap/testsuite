...
Get Process Group Information
    ...
    ${resp} =  Get Request  ${SESSION_NAME}   /nifi-api/process-groups/root/process-groups  headers=${headers}
    Should Be True  ${resp.status_code} < 300
    Log  ${resp.json()}

    Set Test Variable  ${processGrInfo}  &{EMPTY}

    FOR  ${processGroup}  IN  @{resp.json().get('processGroups')}
         ${registryId} =  Run Keyword If  '${processGroupId}' == '${processGroup['id']}'  Set Variable  ${processGroup['versionControlInformation']['registryId']}
         ${flowId} =  Run Keyword If  '${processGroupId}' == '${processGroup['id']}'  Set Variable  ${processGroup['versionControlInformation']['flowId']}
         ${bucketId} =  Run Keyword If  '${processGroupId}' == '${processGroup['id']}'  Set Variable  ${processGroup['versionControlInformation']['bucketId']}
         ${clientId} =  Run Keyword If  '${processGroupId}' == '${processGroup['id']}'  Set Variable  ${processGroup['revision']['clientId']}
         ${version} =  Run Keyword If  '${processGroupId}' == '${processGroup['id']}'  Set Variable  ${processGroup['revision']['version']}
         Run Keyword If  '${processGroupId}' == '${processGroup['id']}'  Set To Dictionary  ${processGrInfo}  registryId=${registryId}  flowId=${flowId}  bucketId=${bucketId}  clientId=${clientId}  version=${version}
    END

   [Return]  ${processGrInfo}
...
#    ${currentProcessGrVersion}  ${_}=  Get Process Group Revision  ${processGroupId}
    ...
    ${processGrInfo}=  Get Process Group Information  ${processGroupId}


    ${data} =  Set Variable  {"versionedFlow": {"flowName": "${flowName}", "bucketId": "${processGrInfo['bucketId']}", "registryId": "${processGrInfo['registryId']}"}, "processGroupRevision": {"clientId": "${CLIENT_ID}", "version": ${processGrInfo['version']}}}
...
#    ${flowId} =  Set Variable  ${resp.json().get('component').get('versionControlInformation').get('flowId')}
    ...
    ${data} =  Set Variable  {"processGroupId": "${processGrInfo['flowId']}"}
...
Delete Process Group
    ...
    ${resp} =  Delete Request  ${SESSION_NAME}  /nifi-api/process-groups/${processGroupId}?version=${processGrInfo['version']}&clientId=${processGrInfo['clientId']}
    Should Be True  ${resp.status_code} < 300
...
Delete Registry Client
    ...
    ${processGrInfo}=  Get Process Group Information  ${processGroupId}
    ${resp} =  Delete Request  ${SESSION_NAME}  /nifi-api/controller/registry-clients/${processGrInfo['registryId']}?version=${processGrInfo['version']}
    Should Be True  ${resp.status_code} < 300
