...
Create Processor
    ...
    ${session}=  Create Session   ${SESSION_NAME}  ${DCAEMOD_SERVER}
    ${headers}=  Create Dictionary  content-type=application/json
    ${data} =  Set Variable  {"revision": {"clientId": "${CLIENT_ID}", "version": 0},"component": {"parentGroupId": "${processGroupId}", "name": "${compSpecName}", "type": "org.onap.dcae.${compSpecName}"}}
...
Specific Test Teardown
    [Arguments]  ${is_process_group_set}  ${is_flow_distributed_set}  ${typeId}  ${blueprintName}  ${is_bp_deployed_set}  ${processGroupId}
    Run Keyword If  ${is_process_group_set}        Wait Until Keyword Succeeds  2 min  5s  Delete Process Group  ${processGroupId}
    Run Keyword If  ${is_flow_distributed_set}     Delete Blueprint From Inventory  ${typeId}
    Run Keyword If  ${is_bp_deployed_set}          Undeploy Service  ${blueprintName}