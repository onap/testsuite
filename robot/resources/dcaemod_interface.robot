...
Is Component Onborded
    ...
    ${resp} =  Get Request  ${alias}  /onboarding/components?name=${COMPONENT_NAME}  ${headers}
    ${compFound)=  Set Variable  ${resp.json().get('component')}
    ${onboardStatus} =  Run Keyword And Return Status  Should Not Be Empty  ${compFound)
...
Onboard Component Spec
    ...
    Wait Until Keyword Succeeds  5 min  20s  Is Component Onborded  ${alias}  ${headers}
    Log  ${resp.json()}
