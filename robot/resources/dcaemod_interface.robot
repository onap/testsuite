...
Create Processor
    ...
    ${session}=  Create Session   ${SESSION_NAME}  ${DCAEMOD_SERVER}
    ${headers}=  Create Dictionary  content-type=application/json
    ${typeName} =  Convert To Title Case  ${compSpecName}
...
Specific Test Teardown
    [Arguments]  ${is_process_group_set}=False  ${is_flow_distributed_set}=False  ${typeId}=${EMPTY}  ${blueprintName}=${EMPTY}  ${is_bp_deployed_set}=False  ${processGroupId}=${EMPTY}
    ...
    Run Keyword If  ${is_process_group_set}        Delete Process Group  ${processGroupId}
