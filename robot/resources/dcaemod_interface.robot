...
Deploy Blueprint From Inventory
    [Arguments]  ${typeName}
    ${componentName} =  Convert To Lower Case  ${COMPONENT_NAME}
    ${resp} =  Wait Until Keyword Succeeds  7 min  20s  Get Blueprint From Inventory  ${typeName}_${componentName}
    ${typeId} =  Set Variable  ${resp.json().get('items')[0].get('typeId')}
    ${data} =  Set Variable  {"serviceTypeId": "${typeId}"}
    Deploy Service  ${data}  ${typeName}_${componentName}
