...
${MUSIC_ENDPOINT}     ${GLOBAL_MUSIC_SERVER_PROTOCOL}://${GLOBAL_INJECTED_MUSIC_IP_ADDR}:${GLOBAL_MUSIC_PORT}
...
Run MUSIC Get Request
     ...
     [Arguments]    ${data_path}
     ${auth}=  Create List  ${GLOBAL_MUSIC_USERNAME}    ${GLOBAL_MUSIC_PASSWORD}
     ${session}=    Create Session 	music	${MUSIC_ENDPOINT}    auth=${auth}
     ${uuid}=    Generate UUID
