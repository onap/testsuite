...
DEFAULT_DISPLAY=":99"
DEFAULT_ROBOT_TEST="-i closedloophealth"
INSTALL_NAME="OpenECOMP_ETE"
DEFAULT_OUTPUT_FOLDER=/share/logs/ClosedLoopHealth/
...
VARIABLES="-v filename:vfw_preload.template"
...
python -m robot.run -L ${LOG_LEVEL} -d ${OUTPUT_FOLDER} ${VARIABLEFILES} ${VARIABLES} ${LISTENERS} -P ${ROBOT_LIBS} ${ROBOT_TAGS} -t "Test Simple Close Loop" $(pwd)/robot/testsuites/closed-loop-health.robot
RET_CODE=$?
